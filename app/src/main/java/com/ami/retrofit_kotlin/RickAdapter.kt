...
class RickAdapter(val dataRick: List<ResultsItem>): RecyclerView.Adapter<RickAdapter.MyViewHolder>() {
    class MyViewHolder (view: View): RecyclerView.ViewHolder(view) {
        ...
        val species = view.findViewById<TextView>(R.id.item_species_rick)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
       holder.nameRick.text = dataRick?.get(position)?.name
        holder.nameRick.text = dataRick?.get(position)?.status
        holder.nameRick.text = dataRick?.get(position)?.species
